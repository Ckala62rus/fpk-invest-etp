...
/**
 * Запрос создания черновика ТЗП (super_admin | trade_admin).
 */
class StoreProcedureRequest extends FormRequest
{
    ...
    /**
     * Правила создания черновика ТЗП.
     *
    ...
    {
        return [
            'type' => ['required', 'string', Rule::in(array_column(ProcedureType::cases(), 'value'))],
    ...
    {
        return [
            'type.required' => 'Тип процедуры обязателен для заполнения.',
            'type.in' => 'Тип процедуры указан неверно.',
            'title.required' => 'Название процедуры обязательно для заполнения.',
            ...
            'company_id.required' => 'Заказчик обязателен для заполнения.',
            'company_id.exists' => 'Указанный заказчик не найден.',
            'classifier_category_id.required' => 'Категория классификатора обязательна для заполнения.',
